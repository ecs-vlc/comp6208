...
validation = sort(validation, less);
...
struct FunctionApprox {
  ...
  void draw(string comment="") {
	...
	label("\Huge " + comment, (0.5, steps[i].y), NW);
	print = true;
      }
  ...
  real validation(pair[] validation) {
    int index = 0;
    real x = steps[1].x;
    real y = steps[0].y;
    real sum = 0.0;
    for(int i=0; i<validation.length; ++i) {
      while (validation[i].x>x) {
	++index;
	y = steps[index].y;
	if (index<steps.length-1) {
      ...
      sum += (validation[i].y-y)*(validation[i].y-y);
    }
    return sqrt(sum/validation.length);
  }
}
...
picture background(pair[] data, bool groundTruth=false) {
  ...
  xaxis(bg, "\Large $x$",  min_x, max_x, Arrow);
  yaxis(bg, "\Large $y$", -0.3, 0.3, Arrow);
...
pair[]  step(pair[] data, picture bg, string s="", bool show=false) {
...
void combined(FunctionApprox fa, pair[] data, string s="") {
  ...
  xaxis("\Large $x$",  min_x, max_x, Arrow);
  yaxis("\Large $y$", -0.3, 0.3, Arrow);
...
pair[] data1 = step(data, bg, "$C_1(x) = h_1(x)$", true);
guide g;
for(int t=0; t<10; ++t) {
  pair p = (t, 100*fapprox.validation(validation));
  write(p);
  g = g--p;
  picture bg1 = background(data1);
  data1 = step(data1, bg1, string(t), true);
}
...
draw(g, red);
xaxis("\Large iteration, $t$",  0, 50, Arrow);
yaxis("\Large Squared Error",  Arrow);

shipout("gradBoost_validation");
